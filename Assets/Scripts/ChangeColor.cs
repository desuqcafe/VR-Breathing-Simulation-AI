...
public class ChangeColor : MonoBehaviour
{
    ...
    void ReadCallback(IAsyncResult ar)
    {
        ...
        else
        {
            ...
            { 
                content = state.colorCode.ToString();
                print($"Read [CHANGECOLOR] {content.Length} bytes from socket.\n Data : {content}");
           //     SetColors(content);
            }
            handler.Close();
        }
...
public float SetColors(string data, out float sineWaveAmplitude) {
  string[] colors = data.Split(',');
  sineWaveAmplitude = float.Parse(colors[3]);
